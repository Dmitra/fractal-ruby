...
class FractalApp < Wx::App
  ...
  def on_init
    ...
    line1 = Line.new(400,750,100,0).absolute
    staple = Fractal.new(line1, "tick", 2)
    ...
    tree1 = Fractal.new(line2, "tree1", 6)
    line3 = Line.new(700,500,50,0).absolute
    pyramid = Fractal.new(line3, "pyramid", 15)
    
    buffer.draw do |surface|        
      pyramid.draw.each_with_index{|a, index| 
...
#         surface.draw_label("#{a.coord[0].to_i}, #{a.coord[1].to_i} -> #{a.coord[2].to_i}, #{a.coord[3].to_i}",Wx::Rect.new(a.coord[0].to_i, a.coord[1].to_i, a.coord[0].to_i+30, a.coord[1].to_i+30))
      } # конец прорисовки
